...
$(document).on("click",".peroutcome", function() {
        ...
        get_("get_kpis",{ outcomeid : outcomeid , region_id_selected : region_id_selected }, function(data) {
            ...
            for(var io = 0; io <= Object.keys(data.values).length-1; io++) {
                $("<li class='perkpi' data-outputid='"+data.values[io].outputid+"'> <div class='row'> <div class='col-md-10'>"+data.values[io].kpi+" </div> <div class='col-md-2'><span class='badge badge-light-success fs-base'><i class='ki-outline ki-arrow-up fs-5 text-success ms-n1'></i>"+data.values[io].thevalue+"</span></div> </div> </li>").appendTo("#kpis_"+outcomeid);
            }
...
function display_info(data, apto = "theoutcomes") {
    ...
    for(var oc = 0; oc <= Object.keys(data.outcomes).length-1; oc++) {
        $("<div class='col-md-12 peroutcome row' id='outcome_"+data.outcomes[oc].outcomeid+"' data-outcomeid='"+data.outcomes[oc].outcomeid+"'><div class='col-md-10'><h5>"+data.outcomes[oc].theoutcome+"</div> <div class='col-md-2'><span class='badge badge-light-success fs-base'><i class='ki-outline ki-arrow-up fs-5 text-success ms-n1'></i>"+data.outcomes[oc].thevalue+"</span></div></h5></div>").appendTo("#"+apto);
    }
}
